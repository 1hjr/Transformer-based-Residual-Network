...
class WindowAttention(nn.Module):
    ...
    def extra_repr(self) -> str:
        return 'dim={self.dim}, win_size={self.win_size}, num_heads={self.num_heads}'
...
class LeFF(nn.Module):
    def __init__(self, dim=64, hidden_dim=256, act_layer=nn.GELU,drop = 0.):
        super().__init__()
...
#########################################
from RB import RBlock
class Residual(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(Residual, self).__init__()

        self.conv=RBlock(in_channel, out_channel)
...
class LeWinTransformerBlock(nn.Module):
    ...
    def extra_repr(self) -> str:
        return "dim={self.dim}, input_resolution={self.input_resolution}, num_heads={self.num_heads}, " \
               "win_size={self.win_size}, shift_size={self.shift_size}, mlp_ratio={self.mlp_ratio}"
...
class BasicLayer(nn.Module):
    ...
    def extra_repr(self) -> str:
        return "dim={self.dim}, input_resolution={self.input_resolution}, depth={self.depth}"
...
class TransformerRB(nn.Module):
    def __init__(self, img_size=256, in_chans=28,
        ...
        self.RB_0 = RB(embed_dim, embed_dim)
        self.encoderlayer_1 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[1],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_1 = RB(embed_dim, embed_dim)
        self.encoderlayer_2 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[2],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_2 = RB(embed_dim, embed_dim)
        self.encoderlayer_3 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[3],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_3 = RB(embed_dim, embed_dim)
        # Decoder
        self.decoderlayer_0 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[4],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_4 = RB(embed_dim, embed_dim)
        
        self.decoderlayer_1 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size ,
                                                img_size),
                            depth=depths[5],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_5 = RB(embed_dim, embed_dim)
        
        self.decoderlayer_2 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size ,
                                                img_size),
                            depth=depths[6],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_6 = RB(embed_dim, embed_dim)
        
        self.decoderlayer_3 = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size ,
                                                img_size),
                            depth=depths[7],
                            ...
                            use_checkpoint=use_checkpoint,
                            token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer)
        self.RB_7 = RB(embed_dim, embed_dim)
        
        self.Tranlayer = BasicLayer(dim=embed_dim,
                            output_dim=embed_dim,
                            input_resolution=(img_size,
                                                img_size),
                            depth=depths[8],
    ...
    def extra_repr(self) -> str:
        return "embed_dim={self.embed_dim}, token_projection={self.token_projection}, token_mlp={self.mlp},win_size={self.win_size}"
    ...
    def forward(self, x, mask=None):
        ...
        Tran0 = self.encoderlayer_0(y,mask=mask)
        RB0 = self.RB_0(Tran0)
        Tran1 = self.encoderlayer_1(RB0,mask=mask)
        RB1 = self.RB_1(Tran1)
        Tran2 = self.encoderlayer_2(RB1,mask=mask)
        RB2 = self.RB_2(Tran2)
        Tran3 = self.encoderlayer_3(RB2,mask=mask)
        RB3 = self.RB_3(Tran3)
        '''

        Tran4 = self.decoderlayer_0(RB3, mask=mask)
        RB4 = self.RB_4(Tran4)
        
        Tran5 = self.decoderlayer_1(RB4,mask=mask)
        RB5 = self.RB_5(Tran5)
        
        Tran6 = self.decoderlayer_2(RB5,mask=mask)
        RB6 = self.RB_6(Tran6)
        
        Tran7 = self.decoderlayer_3(RB6,mask=mask)
        RB7 = self.RB_7(Tran7)
        
        Tran8 = self.Tranlayer(RB7,mask=mask)
        '''

        # Output Projection
        y = self.output_proj(RB3)
        return x + y
